...
export async function GET(request) {
  try {
    ...
    const redirectUri = request.cookies.get('github_oauth_redirect')?.value || '/';
    ...
    const sessionData = {
      ...
      permissions: tokenData.scope ? tokenData.scope.split(',') : ['public_repo', 'repo'],
      userData: {
        name: userData.name,
...
function setSessionCookies(response, sessionData) {
  ...
  response.cookies.set('github_session_id', sessionData.githubId, cookieOptions);
  response.cookies.set('github_username', sessionData.username, cookieOptions);
  response.cookies.set('github_access_token', sessionData.accessToken, cookieOptions);
  
  if (sessionData.refreshToken) {
    response.cookies.set('github_refresh_token', sessionData.refreshToken, cookieOptions);
  }
  
  response.cookies.set('github_token_expiry', sessionData.tokenExpiry.toISOString(), cookieOptions);
  response.cookies.set('github_permissions', sessionData.permissions.join(','), cookieOptions);
  
  // Store user data as JSON (less sensitive)
  response.cookies.set('github_user_data', JSON.stringify(sessionData.userData), {
    ...cookieOptions,
