...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    return {
      ...
      description: pageInfo?.description || data.description || `${pageTitle} page of ${data.name || username}'s portfolio`,
      openGraph: {
        title: `${pageTitle} - ${data.name || username}'s Portfolio`,
        description: pageInfo?.description || data.description || `${pageTitle} page of ${data.name || username}'s portfolio`,
        type: 'website',
...
export default async function PortfolioPageRoute({ params }) {
  ...
  try {
      ...
      return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
          <div className="max-w-md mx-auto text-center">
            ...
            <h1 className="text-xl font-semibold text-gray-900 mb-2">Page Navigation Unavailable</h1>
            <p className="text-gray-600 mb-4">
              Unable to load page navigation for <span className="font-mono">{username}/{repo}</span>
            </p>
            <a 
              href={`/${username}/${repo}`}
    ...
    const pageExists = navigationService.pageExists(pagePath, navigationResult.navigation);
    if (!pageExists) {
      return (
        <div className="min-h-screen flex items-center justify-center bg-gray-50">
          <div className="max-w-md mx-auto text-center">
            ...
            <p className="text-gray-600 mb-4">
              The page <span className="font-mono">{pagePath}</span> does not exist in this portfolio.
            </p>
            <div className="space-y-2">
              ...
              {navigationResult.navigation.pages.length > 0 && (
                <details className="mt-4">
                  <summary className="text-sm text-gray-500 cursor-pointer">Available Pages</summary>
    ...
    // Get page content
    const pageContent = await navigationService.getPageContent(username, repo, pagePath);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="max-w-md mx-auto text-center">
          ...
          <p className="text-gray-600 mb-4">
            An unexpected error occurred while loading the portfolio page.
          </p>
          <a 
            href={`/${username}/${repo}`}
            className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors"
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="max-w-md mx-auto text-center">
        ...
        <p className="text-gray-600 mb-4">
          Unable to load portfolio page from <span className="font-mono">{username}/{repo}/{pagePath}</span>
        </p>
